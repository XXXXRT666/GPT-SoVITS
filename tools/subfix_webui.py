    ...
    with open(g_load_file, "r", encoding="utf-8") as source:
        ...
        for _ in data_list:
            ...
            if len(data) == 4:
                ...
                g_data_json.append({"wav_path": wav_path, "speaker_name": speaker_name, "language": language, "text": text.strip()})
            else:
...
if __name__ == "__main__":
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            batchsize_slider = gr.Slider(minimum=1, maximum=g_batch, value=g_batch, step=1, label="Batch Size", scale=3, interactive=False)
        ...
        btn_delete_audio.click(b_delete_audio, inputs=[*g_checkbox_list], outputs=[index_slider, *g_text_list, *g_audio_list, *g_checkbox_list])

        btn_merge_audio.click(
            b_merge_audio, inputs=[interval_slider, *g_checkbox_list], outputs=[index_slider, *g_text_list, *g_audio_list, *g_checkbox_list]
        )

        btn_audio_split.click(
            b_audio_split, inputs=[splitpoint_slider, *g_checkbox_list], outputs=[index_slider, *g_text_list, *g_audio_list, *g_checkbox_list]
        )
    ...
    demo.launch(server_name="0.0.0.0", inbrowser=True, quiet=True, share=eval(args.is_share), server_port=int(args.webui_port_subfix))
