...
def mel_fn(x):
    return mel_spectrogram_torch(
        x, **{"n_fft": 1024, "win_size": 1024, "hop_size": 256, "num_mels": 100, "sampling_rate": 24000, "fmin": 0, "fmax": None, "center": False}
    )
...
class TTS:
    ...
    def init_bert_weights(self, base_path: str):
        ...
        self.bert_tokenizer: PreTrainedTokenizer = AutoTokenizer.from_pretrained(base_path)  # type: ignore
        self.bert_model: BertForMaskedLM = AutoModelForMaskedLM.from_pretrained(base_path)
    ...
    def to_batch(
        ...
        for batch_idx, index_list in enumerate(batch_index_list):
            ...
            if batch_idx == len(batch_index_list) - 1:
                while len(phones_list) < batch_size:
                    phones_list.append(phones_list[-1])
                    phones_len_list.append(phones_len_list[-1])
                    all_phones_list.append(all_phones_list[-1])
                    all_phones_len_list.append(all_phones_len_list[-1])
                    all_bert_features_list.append(all_bert_features_list[-1])
                    norm_text_batch.append(norm_text_batch[-1])
    ...
    def run(self, inputs: dict):
        ...
        try:
            ...
            for idx, item in enumerate(data):
                ...
                pred_semantic_list = self.t2s_model.model.forward(
                    all_phoneme_ids,
                    all_phoneme_lens,
                    prompt,
                    all_bert_features,
                ...
                if idx == len(data) - 1 and res_len:
                    pred_semantic_list = pred_semantic_list[:res_len]
                t4 = ttime()
                t_34 += t4 - t3
    ...
    def v3_synthesis(self, semantic_tokens: torch.Tensor, phones: torch.Tensor, speed: float = 1.0, sample_steps: int = 32):
