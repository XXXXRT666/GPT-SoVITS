...
def change_sovits_weights(sovits_path, prompt_language=None, text_language=None):
    ...
    if prompt_language is not None and text_language is not None:
        ...
        }, prompt_text_update, prompt_language_update, text_update, text_language_update, {"__type__": "update", "visible": visible_sample_steps}, {
            "__type__": "update",
            "visible": visible_inp_refs,
        }, {
            "__type__": "update",
            "value": False,
    ...
    with gr.Group():
        ...
        with gr.Row():
            GPT_dropdown = gr.Dropdown(
                label=i18n("GPT模型列表"), choices=sorted(GPT_names, key=custom_sort_key), value=gpt_path, interactive=True, scale=14
            )
            SoVITS_dropdown = gr.Dropdown(
                label=i18n("SoVITS模型列表"), choices=sorted(SoVITS_names, key=custom_sort_key), value=sovits_path, interactive=True, scale=14
            )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=13):
                ref_text_free = gr.Checkbox(
                    label=i18n("开启无参考文本模式。不填参考文本亦相当于开启。v3暂不支持该模式，使用了会报错。"),
                    value=False,
                    interactive=True,
                    show_label=True,
                    scale=1,
                ...
                gr.Markdown(
                    html_left(i18n("使用无参考文本模式时建议使用微调的GPT，听不清参考音频说的啥(不晓得写啥)可以开。<br>开启后无视填写的参考文本。"))
                )
            ...
            with gr.Column(scale=14):
                ...
                sample_steps = (
                    gr.Radio(label=i18n("采样步数,如果觉得电,提高试试,如果觉得慢,降低试试"), value=32, choices=[4, 8, 16, 32], visible=True)
                    if model_version == "v3"
                    else gr.Radio(label=i18n("采样步数,如果觉得电,提高试试,如果觉得慢,降低试试"), value=8, choices=[4, 8, 16, 32], visible=False)
                )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=7):
                ...
                if_freeze = gr.Checkbox(
                    label=i18n("是否直接对上次合成结果调整语速和音色。防止随机性。"), value=False, interactive=True, show_label=True, scale=1
                )
        ...
        SoVITS_dropdown.change(
            ...
            [prompt_language, text_language, prompt_text, prompt_language, text, text_language, sample_steps, inp_refs, ref_text_free],
        )
