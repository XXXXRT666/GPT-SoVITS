...
def open_asr(asr_inp_dir, asr_opt_dir, asr_model, asr_model_size, asr_lang, asr_precision):
    ...
    if p_asr == None:
        ...
        }, {"__type__": "update", "value": output_file_path}, {"__type__": "update", "value": asr_inp_dir}
    else:
...
def open_slice(inp, opt_root, threshold, min_length, min_interval, hop_size, max_sil_kept, _max, alpha, n_parts):
    ...
    if os.path.exists(inp) == False:
        yield "输入路径不存在", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}, {"__type__": "update"}, {
            "__type__": "update"
        }, {"__type__": "update"}
        return
    ...
    if ps_slice == []:
        for i_part in range(n_parts):
            ...
            ps_slice.append(p)
        yield "切割执行中", {"__type__": "update", "visible": False}, {"__type__": "update", "visible": True}, {"__type__": "update"}, {
            "__type__": "update"
        }, {"__type__": "update"}
        for p in ps_slice:
            p.wait()
        ...
        yield "切割结束", {"__type__": "update", "visible": True}, {"__type__": "update", "visible": False}, {
            "__type__": "update",
            "value": opt_root,
        }, {"__type__": "update", "value": opt_root}, {"__type__": "update", "value": opt_root}
    else:
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        cnhubert_base_dir = gr.Textbox(
                            label=i18n("预训练的SSL模型路径"), value="GPT_SoVITS/pretrained_models/chinese-hubert-base", interactive=False, lines=2
                        )
                    with gr.Row():
                ...
                with gr.Row():
                    ...
                    with gr.Row():
                        pretrained_s2G_ = gr.Textbox(
                            label=i18n("预训练的SoVITS-G模型路径"), value=pretrained_sovits_name[int(version[-1]) - 1], interactive=False, lines=2
                        )
                    with gr.Row():
                    ...
                    with gr.Column():
                        with gr.Column():
                            if_save_latest = gr.Checkbox(
                                label=i18n("是否仅保存最新的ckpt文件以节省硬盘空间"), value=True, interactive=True, show_label=True
                            )
