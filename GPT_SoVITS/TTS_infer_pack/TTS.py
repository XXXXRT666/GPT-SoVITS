...
class TTS:
    ...
    def _init_models(self):
        ...
        self.init_bert_weights(self.speaker.bert)
        self.init_cnhuhbert_weights(self.speaker.cnhubert)
    ...
    def init_cnhuhbert_weights(self, base_path: str):
        ...
        if self.configs.fp16 and str(self.configs.device) != "cpu":
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        self.hps.max_sec = config["data"]["max_sec"]
    ...
    def set_ref_audio(self, ref_audio_path: str):
        ...
        if not ref_audio_path:
            raise ValueError("ref_audio_path can not be empty")
        if not os.path.exists(ref_audio_path):
            raise ValueError(f"{ref_audio_path} not exists")
        if self._set_ref_audio_path(ref_audio_path):
            return
        self._set_prompt_semantic(ref_audio_path)
        self._set_ref_spec(ref_audio_path)

    def _set_ref_audio_path(self, ref_audio_path):
        if self.prompt_cache["ref_audio_path"] == ref_audio_path:
            return True
        self.prompt_cache["ref_audio_path"] = ref_audio_path
        return False

    def _set_ref_spec(self, ref_audio_path):
        spec = self.get_ref_spec(ref_audio_path)
    ...
    def get_ref_spec(self, ref_audio_path):
        audio = load_audio(ref_audio_path, int(self.hps.data.sampling_rate))
    ...
    def _set_prompt_semantic(self, ref_wav_path: str):
        zero_wav = np.zeros(
            ...
            dtype=np.float16 if self.configs.fp16 else np.float32,
        )
        with torch.no_grad():
    ...
    def to_batch(
        ...
        for _, index_list in enumerate(batch_index_list):
            ...
            # all_bert_features_list = [self.t2s_model.model.bert_proj(item.to(self.t2s_model.device).transpose(0, 1))
            # for item in all_bert_features_list]
    ...
    def recovery_order(self, data: list, batch_index_list: list) -> list:
        ...
        Args:
            data (list[list[NDArray]]]: the out of order audio .
            batch_index_list (list[list[int]]): the batch index list.

        Returns:
            list[NDArray]: the data in the original order.
        """
    ...
    def run(self, tts_request: TTSRequest, progress_tracker) -> Generator[TTSResponse, None, None]:
        ...
        returns:
            Tuple[int, NDArray]: sampling rate and audio data.
        """

        ########## variables initialization ###########
        self.stop_flag: bool = False
        text: str = tts_request.text
        text_lang: str = tts_request.text_lang
        top_k: int = tts_request.top_k
        top_p: float = tts_request.top_p
        temperature: float = tts_request.temperature
        text_split_method: str = tts_request.text_split_method
        batch_size = tts_request.batch_size
        batch_threshold = tts_request.batch_threshold
        speed_factor = tts_request.speed_factor
        split_bucket = tts_request.split_bucket
        return_fragment = tts_request.return_fragment
        fragment_interval = tts_request.fragment_interval
        seed = tts_request.seed
        repetition_penalty = tts_request.repetition_penalty
        no_prompt_text = tts_request.no_prompt_text
        infer_panel = self.t2s_model.model.infer_panel_batch_infer if tts_request.parallel_infer else self.t2s_model.model.infer_panel_naive_batched
        ...
        try:
            ...
            for item in progress_tracker(data):
                ...
                pred_semantic_list, idx_list = infer_panel(
                    all_phoneme_ids,
                    all_phoneme_lens,
                    prompt,
                    all_bert_features,
                    ...
                    early_stop_num=self.hps.hz * self.hps.max_sec,
                    max_len=max_len,
    ...
    def audio_postprocess(
        ...
        return_zero_wav=False,
    ):
        zero_wav = torch.zeros(int(self.hps.data.sampling_rate * fragment_interval), dtype=self.precision, device=self.configs.device)

        if return_zero_wav:
            processed_audio = zero_wav.cpu().numpy().astype(np.int16)
            return sr, processed_audio

        processed_audio: list[list[NDArray]] = []
        for batch in audio:
            ...
            for audio_fragment in batch:
                ...
                processed_audio[-1].append(audio_fragment.cpu().numpy())

        if split_bucket:
            processed_audio: list[NDArray] = self.recovery_order(processed_audio, batch_index_list)
        else:
            # audio = [item for batch in audio for item in batch]
            processed_audio: list[NDArray] = sum(processed_audio, [])

        processed_audio: NDArray = np.concatenate(processed_audio, 0)
...
# def speed_change(input_audio:NDArray, speed:float, sr:int): # Deprecated
