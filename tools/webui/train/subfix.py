...
class Subfix:
    ...
    def __init__(self, i18n: I18nAuto):
        ...
        with gr.Row(equal_height=True):
            with gr.Column(scale=2, min_width=160):
                self.batch_size_slider = gr.Slider(minimum=4, maximum=20, step=2, value=self.batch_size, label=i18n("每页音频条数"))
            with gr.Column(scale=1, min_width=160):
                self.merge_audio_button = gr.Button(value=i18n("合并选中音频"))
            with gr.Column(scale=1, min_width=160):
                self.delete_audio_button = gr.Button(value=i18n("删除选中音频"))
        self.render_text_area = gr.render(
            inputs=[
                self.index_slider,
                self.batch_size_slider,
            ],
            triggers=[
                self.batch_size_slider.change,
            ],
        )(
            self.render_text_area
        )  # Decorator can't work with self
    ...
    def load_list(self, list_path: str):
        ...
        for idx, transcriptions in enumerate(list_data):
            ...
            self.transcriptions_list.append(
                [audio_name, audio_folder, LANGUAGE_MAP.get(text_language.upper(), text_language.upper()), text.strip("\n").strip()]
            )
    ...
    def submit(self, *input):
        with LOCK:
            ...
            if isinstance(result, SubfixErr):
                gr.Warning(result.error, title="Subfix Error")
    ...
    def merge_audio(self, index, *selected):
        ...
        if len(merge_index) < 2:
            ...
            first_itm_path = (
                f"{os.path.splitext(self.transcriptions_list[first_itm_index][0])[0]}_{str(datetime.datetime.now().strftime(r'%Y%m%d_%H%M%S'))}.wav"
            )
            final_audio_list = []
    ...
    def render_text_area(self, index, batch_size):
        i18n = self.i18n
        self.textboxes = []
        self.audios = []
        self.languages = []
                ...
                if i <= self.max_index:
                    with gr.Column(scale=2, min_width=160):
                        textbox_tmp = gr.Textbox(
                            value=self.transcriptions_list[i][3], label=i18n("Text") + f" {i}", lines=2, max_lines=3, interactive=True
                        )
                    with gr.Column(scale=1, min_width=160):
                        audio_tmp = gr.Audio(
