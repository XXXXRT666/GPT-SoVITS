...
class TTSRequestAPI(BaseModel):
    ...
    ref_audio_path: Optional[str] = Field(
        None, description="Reference Audio Path for TTS, Optional If Provided in Speaker Setting(Prompt)", examples=[None]
    )
    prompt_text: Optional[str] = Field(None, description="Text of Reference Audio", examples=[None])
    prompt_lang: Optional[str] = Field(None, description="Language of Reference Audio", examples=[None])
    aux_ref_audio_paths: Optional[Annotated[List[str], Field(min_items=0, max_items=10)]] = Field(
        None, description="Auxiliary Reference Audio Paths for Voice Fusion", examples=[[]]
    )
    ...
    seed: Optional[Annotated[int, Field(ge=-1)]] = Field(-1, description="Random Seed", examples=[-1])
...
class SpeakerAPI(BaseModel):
    speaker_name: str = Field(description="Name of Speaker", examples=[""])
    t2s_path: Optional[str] = Field(None, description="Path of the GPT Model, GPT V2 model if unset", examples=[None])
    vits_path: Optional[str] = Field(None, description="Path of the SoVITS Model, SoVITS V2 model if unset", examples=[None])
    version: Literal["v1", "v2"] = Field("v2", description="Version of the Model", examples=["v2"])
    ...
    aux_ref_audio_paths: Annotated[List[str], Field(min_items=0, max_items=10)] = Field(default_factory=list, examples=[[]])
