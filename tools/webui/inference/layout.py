...
def build_app(tts_engine: TTSEngine, compile: bool = False):  # pylint: disable=redefined-builtin
    ...
    with gr.Blocks(
        theme=seafoam,
        title="GPT-SoVITS Inference WebUI",
        js=js,
        css=css,
        ...
        with gr.Row(equal_height=True):
            with gr.Column():
                GPT_dropdown = gr.Dropdown(
                    choices=get_gpt_paths(),
                    value=tts_engine.speaker.t2s_path,
                    label=i18n("GPT 模型列表"),
                    interactive=True,
                    scale=1,
                )
            with gr.Column():
                SoVITS_dropdown = gr.Dropdown(
                    choices=get_sovits_paths(),
                    value=tts_engine.speaker.vits_path,
        ...
        with gr.Row(equal_height=True):
            with gr.Column():
                speakers_dropdown = gr.Dropdown(
                    choices=list(tts_engine.list_speaker()),
                    value=configs.speaker_name,
                    label=i18n("说话人列表"),
                    interactive=True,
                )
            with gr.Column():
                new_speaker_name = gr.Textbox(label=i18n("新说话人名称"), interactive=True)
            with gr.Column():
        ...
        with gr.Row(equal_height=True):
            with gr.Column():
                ...
                with gr.Row(equal_height=True):
                    ...
                    with gr.Row(equal_height=True):
                        ref_free_mode = gr.Checkbox(
                            info=info_html.format(i18n("无参考文本模式")),
                            label=i18n("勾选后忽略已填入参考文本"),
                        )
            with gr.Column():
                text = gr.Textbox(
                    label=i18n("目标文本"),
        ...
        with gr.Row(equal_height=True):
            with gr.Column():
                batch_size = gr.Slider(
                    ...
                    step=1,
                    label=i18n("批处理大小"),
                    interactive=not compile,
                )
                fragment_interval = gr.Slider(
                    minimum=0.1,
                    maximum=1.0,
                    value=configs.fragment_interval,
                    step=0.1,
                    label=i18n("段间间隔"),
                    interactive=True,
                )
                ...
                repetition_penalty = gr.Slider(
                    minimum=0,
                    maximum=2,
                    value=configs.reprtition_penalty,
                    step=0.05,
                    label=i18n("reprtition_penalty"),
                    interactive=True,
                )
            with gr.Column():
                with gr.Row(equal_height=True):
                ...
                with gr.Row(equal_height=True):
                    with gr.Row(equal_height=True):
                        cut_methods = gr.Dropdown(
                            choices=list(CUT_METHODS_MAPPING.keys()),
                            value=i18n("cut1"),
                            label=i18n("切分方式"),
                            interactive=True,
                        )
