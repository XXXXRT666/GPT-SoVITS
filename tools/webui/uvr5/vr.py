...
from tools.webui.uvr5.lib.utils import inference
...
class AudioPre:
    def __init__(self, agg, model_path, device, is_half, tta=False):
        ...
        mp = ModelParameters("tools/webui/uvr5/lib/lib_v5/modelparams/4band_v2.json")
    ...
    def process(self, music_file, ins_root=None, vocal_root=None, audio_format="flac", is_hp3=False):
        ...
        if ins_root is not None:
            ...
            if audio_format in ["wav", "flac"]:
                ...
                path = os.path.join(
                    ins_root,
                    head + f"{name}_{self.data['agg']}.wav",
                )
                sf.write(
                    path,
                    (np.array(wav_instrument) * 32768).astype(np.int16),
                ...
                if os.path.exists(path):
                    opt_audio_format_path = f"{path[:-4]}.{audio_format}"
        ...
        if vocal_root is not None:
            ...
            if audio_format in ["wav", "flac"]:
                ...
                if os.path.exists(path):
                    opt_audio_format_path = path[:-4] + ".%s" % audio_format
                    os.system("ffmpeg -i %s -vn %s -q:a 2 -y" % (path, opt_audio_format_path))
                    if os.path.exists(opt_audio_format_path):
                        try:
                            os.remove(path)
                        except:
...
class AudioPreDeEcho:
    def __init__(self, agg, model_path, device, is_half, tta=False):
        ...
        mp = ModelParameters("tools/webui/uvr5/lib/lib_v5/modelparams/4band_v3.json")
    ...
    def process(self, music_file, vocal_root=None, ins_root=None, audio_format="flac", is_hp3=False):  # 3个VR模型vocal和ins是反的
        if ins_root is None and vocal_root is None:
            return "No save root."
        name = os.path.basename(music_file)
        if ins_root is not None:
