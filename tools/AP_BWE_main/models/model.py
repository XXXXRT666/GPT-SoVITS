...
class MultiResolutionAmplitudeDiscriminator(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.discriminators = nn.ModuleList([DiscriminatorAR(resolution=r, num_embeddings=num_embeddings) for r in resolutions])
...
class DiscriminatorAR(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cond_embedding_id: torch.Tensor = None) -> Tuple[torch.Tensor, List[torch.Tensor]]:
        fmap = []
        x = x.squeeze(1)
...
class MultiResolutionPhaseDiscriminator(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.discriminators = nn.ModuleList([DiscriminatorPR(resolution=r, num_embeddings=num_embeddings) for r in resolutions])
